...
## Scenario 6: Live session rejoin, auto-advance, and safe close

1. Open `/live-session` and launch or rejoin a non-finished room.
...
6. Confirm the active red teacher canvas exposes Audio language, Next CCI, Auto-play open turn, Auto-preview next, Replay Prompt, and Speech Settings controls while the round is open.
...
Expected result: rooms are resumable from `/live-session`, next turns advance consecutively in the same teacher canvas without stale-response skips, teacher can choose auto or manual audio for both open turns and next-preview screens, active-round settings remain available in the red canvas, and learner pads turn off when the teacher exits or finishes.
